...
CREATE VIEW AdventureWork.sales_summary AS (
	...
	FROM (
			SELECT *, RANK () OVER (PARTITION BY TerritoryKey ORDER BY OrderQuantity DESC) AS rnk
			FROM (
...
T2 AS (
	...
	FROM (
			SELECT TerritoryKey, ProductKey, SUM(ReturnQuantity) AS ReturnQuantity
		...
		LEFT JOIN (
			...
			FROM sales
			GROUP BY 1, 2
		) sales ON sales.ProductKey = sreturn.ProductKey
