...
WITH sales AS ((SELECT * FROM AdventureWork.sales_2017)
				UNION ALL 
                (SELECT * FROM AdventureWork.sales_2016)
                UNION ALL 
                (SELECT * FROM AdventureWork.sales_2015))
SELECT p.ProductName, SUM(OrderQuantity) AS TotalQuantity,
...
FROM (SELECT ProductKey, SUM(OrderQuantity) AS OrderQuantity
		FROM sales
        GROUP BY ProductKey) sale
LEFT JOIN AdventureWork.products p ON p.ProductKey = sale.ProductKey
GROUP BY ProductName, TotalRevenue, TotalProfit
...
HAVING SUM(OrderQuantity) = (SELECT MAX(OrderQuantity)
...
FROM (SELECT *,
		RANK () OVER (PARTITION BY TerritoryKey ORDER BY OrderQuantity DESC) AS rnk
		FROM (
			SELECT TerritoryKey, ProductKey, SUM(OrderQuantity) AS OrderQuantity
				FROM AdventureWork.sales_2017
				GROUP BY TerritoryKey, ProductKey) sale
	 ) sale_rank
...
WITH sales AS ((SELECT * FROM AdventureWork.sales_2017)
                ...
                UNION ALL 
                (SELECT * FROM AdventureWork.sales_2015)),
T2 AS (SELECT sreturn.TerritoryKey, sreturn.ProductKey, sreturn.ReturnQuantity, sales.OrderQuantity 
		FROM (SELECT TerritoryKey, ProductKey, SUM(ReturnQuantity) AS ReturnQuantity
				FROM AdventureWork.sale_returns
				GROUP BY 1,2) sreturn
				LEFT JOIN (SELECT TerritoryKey, ProductKey, SUM(OrderQuantity) AS OrderQuantity
							FROM sales 
                            GROUP BY 1,2) sales 
							ON sales.ProductKey = sreturn.ProductKey
							AND sales.TerritoryKey = sreturn.TerritoryKey)

SELECT p.ProductName, st.Region, st.Country, ROUND((ReturnQuantity*100)/OrderQuantity,2) AS ReturnRate
	FROM T2
	LEFT JOIN AdventureWork.products p
    ON T2.ProductKey = p.ProductKey
    LEFT JOIN AdventureWork.sale_territories st 
    ON st.SalesTerritoryKey = T2.TerritoryKey
ORDER BY st.Region